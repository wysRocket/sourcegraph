...
func getOrCreateUser(ctx context.Context, db database.DB, p *Provider, idToken *oidc.IDToken, userInfo *oidc.UserInfo, claims *userClaims, usernamePrefix string) (newUserCreated bool, _ *actor.Actor, safeErrMsg string, err error) {
	if userInfo.Email == "" {
